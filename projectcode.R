...
# ============================================================
# Poster-friendly comparison of initial background rates
# ============================================================
...
# Make method names shorter for the legend
bg_rate_three_methods <- bg_rate_three_methods %>%
  mutate(
    method_short = case_when(
      method == "Gardner-Knopoff window-based" ~ "Gardner-Knopoff",
      method == "Nearest-neighbour" ~ "Nearest-neighbour",
      method == "Reasenberg-style" ~ "Reasenberg",
      TRUE ~ method
    )
  )

p_bg_three_poster <- ggplot(
  bg_rate_three_methods,
  aes(
    x = month,
    y = background_rate,
    color = method_short,
    linetype = method_short
  )
) +
  geom_line(linewidth = 1.1, alpha = 0.9) +
  scale_color_manual(
    values = c(
      "Gardner-Knopoff" = "#1B9E77",
      "Nearest-neighbour" = "#D95F02",
      "Reasenberg" = "#7570B3"
    )
  ) +
  ...
  labs(
    title = "Comparison of Initial Background Rates",
    subtitle = "Declustering-based initial values for non-stationary ETAS fitting",
    x = "Time",
    y = "Background events per month",
    color = "Declustering method",
    linetype = "Declustering method"
  ) +
...
print(p_bg_three_poster)

ggsave(
  filename = "poster_initial_background_rate_three_declustering_methods_Mc_1_7.png",
  plot = p_bg_three_poster,
  width = 12,
  height = 7,
  dpi = 300
)
